    ...
    > ShardTree<S, DEPTH, SHARD_HEIGHT>
{
    ...
    /// Marks the checkpoint with the given identifier as retained, exempting it from automatic
    ...
    ///
    /// BACKPORT (retention, from upstream `main`): only stores that override the retention
    /// methods of [`ShardStore`] support this; against a store using the trait's no-op
    /// defaults this call records nothing and pruning behaves exactly as before.
    ...
    fn prune_excess_checkpoints(&mut self) -> Result<(), ShardTreeError<S::Error>> {
        ...
        // BACKPORT (retention): checkpoints in the explicit retention set are pinned — they
        // are excluded from the `max_checkpoints` budget and are never removed by automatic
        // pruning. The budget therefore applies only to the non-retained checkpoints.
        let retained = self
            .store
...
mod tests {
    ...
    /// BACKPORT (retention): builds a tree (`max_checkpoints == 3`) containing a `Marked` leaf
    /// at position 2 and an explicitly-retained "anchor" checkpoint (id 1) at position 5, then
    /// advances the tree across the shard boundary while adding more than `max_checkpoints`
    /// additional checkpoints, so that checkpoint 1 would ordinarily be pruned were it not
    /// retained.
